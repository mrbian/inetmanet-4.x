...
#include "inet/wirelesspan/routing/rpl/TrickleTimer.h"
...
public:
  ...
  private:
    ...
    double clKickoffTimeout = 0; // timeout for auto-triggering phase II of CL SF
    uint8_t daoRtxCtn = 0;
    uint8_t daoRtxThresh = 0;
    bool isRoot = false;
    bool daoEnabled = false;
    bool storing = false;
    bool pDaoAckEnabled = false;
    bool hasStarted = false;
    bool allowDodagSwitching = false;
    bool pUnreachabilityDetectionEnabled = false;
    bool pShowBackupParents = false;
    bool pAllowDaoForwarding = false;
    bool pJoinAtSinkAllowed = false;
    uint16_t rank = 0;
    uint8_t dtsn = 0;
...
//    std::map<Ipv6Address, std::pair<cMessage *, uint8_t>> pendingDaoAcks;
    ...
    bool floating = false;
    bool verbose = false;
    bool pUseWarmup = false;
    bool pLockParent = false;
    bool isLeaf = false;
    bool isMobile = false;
    uint8_t detachedTimeout = 2; // temporary variable to suppress msg processing after just leaving the DODAG, set the value to manually suppressing previous DODAG info [RFC 6550, 8.2.2.1]
    cMessage *detachedTimeoutEvent = nullptr; // temporary msg corresponding to triggering above functionality
    cMessage *daoAckTimeoutEvent = nullptr; // temporary msg corresponding to triggering above functionality
    uint8_t prefixLength = 128;
    Coord position = Coord::NIL;
    uint64_t selfId = 0;    // Primary IE MAC address in decimal
...
//    void updateRoutingTable(const Dao *dao);
    ...
    mutable cLineFigure *prefParentConnector = nullptr;
    ...
    IMobility *parentMobilityMod = nullptr; // preferred parent mobility module
