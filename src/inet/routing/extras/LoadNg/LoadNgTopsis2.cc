...
simsignal_t LoadNgTopsis2::recomputeSignal = registerSignal("LoadNgRecomputeSignal");
simsignal_t LoadNgTopsis2::nextRecSignal = registerSignal("LoadNgnextRecSignal");
simtime_t LoadNgTopsis2::nextRecTime;


Define_Module(LoadNgTopsis2);
...
inline std::ostream& operator<<(std::ostream& out, const TopsisLoadNgNeigbors2::Cost& d)
{
...
void LoadNgTopsis2::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        if (par("checkStationary") && !par("stationaryNetwork").boolValue()) {
            checkStationary = true;
            //jitterPar2 = &par("jitter2");
            auto node = getContainingNode(this);
            auto mob = node->getSubmodule("mobility");
            auto rw = dynamic_cast<IMobility *>(mob);
            if (rw != nullptr && rw->isStationary())  {
                stationary = true;
                stationaryNodes.push_back(this->getSelfIPAddress());
            }
            else
                mobileNodes.push_back(this->getSelfIPAddress());
...
void LoadNgTopsis2::actualizeDelayed(Packet *pkt) {
...
void LoadNgTopsis2::handleMessageWhenUp(cMessage *msg)
{
    checkNeigList(msg);
...
INetfilter::IHook::Result LoadNgTopsis2::ensureRouteForDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
    ...
    if (destAddr.isBroadcast() || routingTable->isLocalAddress(destAddr) || destAddr.isMulticast())
        ...
        if (!sourceAddr.isUnspecified() && !routingTable->isLocalAddress(sourceAddr)) {
            ...
            if (routeOrigin) {
                ...
                if (it != macToIpAddress.end()) {
                    ...
                    simtime_t timeOut = simTime() + activeRouteTimeout;
                    if (checkStationary) {
                        auto data = check_and_cast<LoadNgRouteData *>(routeOrigin->getProtocolData());
                        if (data->getStableRoute())
                            timeOut = simTime() + 3*activeRouteTimeout;

                    }
                    updateValidRouteLifeTime(senderAddr, timeOut);
                    updateValidRouteLifeTime(sourceAddr, timeOut);
                }
            }
        }
        ...
        if (isActive && !route->getNextHopAsGeneric().isUnspecified()) {
            ...
            if (checkStationary) {
                auto data = check_and_cast<LoadNgRouteData *>(route->getProtocolData());
                if (data->getStableRoute())
                    timeOut = simTime() + 3*activeRouteTimeout;
            }

            updateValidRouteLifeTime(destAddr, timeOut);
            updateValidRouteLifeTime(route->getNextHopAsGeneric(), timeOut);

            auto it = specialPaths.find(destAddr);
            if (it != specialPaths.end())
                it->second.lifetime = timeOut;
...
LoadNgTopsis2::LoadNgTopsis2()
{
}

bool LoadNgTopsis2::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}


void LoadNgTopsis2::periodicRreqTigger()
{
...
void LoadNgTopsis2::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address LoadNgTopsis2::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void LoadNgTopsis2::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void LoadNgTopsis2::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void LoadNgTopsis2::sendRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive, const double &cost, const double &delay)
{
...
const Ptr<Rreq> LoadNgTopsis2::createRREQ(const L3Address& destAddr)
{
    ...
    if (stationary && checkStationary)
        rreqPacket->setStableRoute(true);
    if (par("stationaryNetwork").boolValue())
        rreqPacket->setStableRoute(true);
    auto it1 = std::find(stationaryNodes.begin(),stationaryNodes.end(), destAddr);
    auto it2 = std::find(stationaryNodes.begin(),stationaryNodes.end(), getSelfIPAddress());
    if (it1 != stationaryNodes.end() && it2 != stationaryNodes.end())
        StableSourceDest++;
    else
        NoStableSourceDest++;
...
const Ptr<Rrep> LoadNgTopsis2::createRREPSp(const L3Address &dest, const SpecialPath &path)
{
...
const Ptr<Rrep> LoadNgTopsis2::createRREP(const Ptr<Rreq>& rreq)
{
...
void LoadNgTopsis2::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr, const MacAddress &macSenderAddress)
{
...
//        oldSeqNum = itSeq->second;
    ...
    if (rrep->getHopCount() > 0 && !routingTable->isLocalAddress(rrep->getDestAddr())) {
        ...
        if (destRoute) {
            ...
            for (int i = 0; i < (int)rrep->getPathAddressArraySize(); i++) {
    ...
    if (rrep->getAccumulate()) {
        ...
        if (!routingTable->isLocalAddress(destAddress)) {
            if (rrep->getAccumulateAddressArraySize() == 0)
                ...
                if (routingTable->isLocalAddress(rrep->getOriginatorAddr())) {
                    ...
                    // Extract next hop
                    for (int i = 0; i < (int)rrep->getAccumulateAddressArraySize(); i++) {
        ...
        if (!routingTable->isLocalAddress(destAddress)) {
            ...
            if(rrep->getIncludeRoute()) {
                ...
                for (int i = 0; i < (int)rrep->getRouteArraySize(); i ++) {
    ...
    */
    ...
    double recpower = NaN;
    int numNeigh = -1;
    bool stable = par("stationaryNetwork").boolValue();
    ...
    if (itNeig != neighbors.end()) {
       ...
       // val = itNeig->second.metricToNeig;
        ...
        if (checkStationary) {
            if (itNeig->second.stationary && stationary)
                stable = true;
            else
                stable = false;
        }
    ...
    if (itTopsisOrg != topsisResults.end() && itTopsisOrg->second.nextAddress != sourceAddr) {
        ...
        stable = itTopsisOrg->second.stable;
    ...
    rrep->setStableRoute(rrep->getStableRoute() && stable);
    if (!isnan(etx)) {
    ...
    bool stableLink = false;
    if (par("stationaryNetwork"))
        stableLink = true;
    simtime_t timeOut = simTime() + activeRouteTimeout;
    if (checkStationary) {
        auto it = this->neighbors.find(sourceAddr);
        if (it != neighbors.end())
            stableLink = stationary && it->second.stationary;
        if (stableLink)
            timeOut = simTime() + 3 * activeRouteTimeout;
    ...
    if (!previousHopRoute || previousHopRoute->getSource() != this) {
        // create without valid sequence number
        previousHopRoute = createRoute(sourceAddr, sourceAddr, 1, -1, true, timeOut, metricType, metricNeg);
        if (stableLink) {
            auto newProtocolData = check_and_cast<LoadNgRouteData *>(previousHopRoute->getProtocolData());
            newProtocolData->setStableRoute(stableLink);
        }
    }
    else {
        ...
        if (itTopsis != topsisResults.end()) {
            if (itTopsis->second.nextAddress == sourceAddr)
                updateRoutingTable(previousHopRoute, sourceAddr, 1, loadNgRouteData->getDestSeqNum(), true, timeOut, metricType, metric);
            else {
                stableLink = false;
                if (par("stationaryNetwork"))
                    stableLink = true;
                simtime_t timeOut = simTime() + activeRouteTimeout;
                if (checkStationary) {
                    stableLink = itTopsis->second.stable;
                    if (stableLink)
                        timeOut = simTime() + par("stableRoutesPrimeTimeOut") * activeRouteTimeout;
                }
                updateRoutingTable(previousHopRoute, itTopsis->second.nextAddress, itTopsis->second.numHops, loadNgRouteData->getDestSeqNum(), true, timeOut, metricType, itTopsis->second.etx);
            }

        }
        else
            updateRoutingTable(previousHopRoute, sourceAddr, 1, loadNgRouteData->getDestSeqNum(), true, timeOut, metricType, metric);
        loadNgRouteData->setStableRoute(stableLink);
    }
    ...
    auto topsisCost = topsis.addCost(
                        ...
                        rrep->getHopCount(), rrep->getDelay(), rrep->getStableRoute());
    ...
    std::deque<TopsisLoadNgNeigbors2::CostAndRange> results;
    topsis.clear();
    ...
    timeOut = simTime() + activeRouteTimeout;
    if (checkStationary) {
        if (rrep->getStableRoute())
            timeOut = simTime() + par("stableRoutesPrimeTimeOut") * activeRouteTimeout;
    }
    ...
    if (originRoute && originRoute->getSource() == this) {    // already exists
        ...
        if (orgRouteData->getDestSeqNum() == -1 || seqNum > orgRouteData->getDestSeqNum()) {
            ...
            updateRoutingTable(originRoute, prevNodeOrigin, newHopCount, seqNum, true, timeOut, metricType, metric);
        }
        else {
            ...
            if (seqNum == orgRouteData->getDestSeqNum() && !orgRouteData->isActive()) {
                actualizeCostRrep(rrep, orgRouteData);
                updateRoutingTable(originRoute, sourceAddr, newHopCount, seqNum, true, timeOut, metricType, metric);
            }
            ...
            //      smaller than the hop count in route table entry.
                ...
                if (!results.empty()) {
                    if (results.front().index == (int)results.size() - 1) { // it is the information of this RREQ that is the latest in to be included
                        actualizeCostRrep(rrep, orgRouteData);
                        updateRoutingTable(originRoute, sourceAddr, newHopCount, seqNum, true, timeOut, metricType, metric, TOPSIS_UPDATE);
                    }
                }
            }
        }
       ...
       // orgRouteData->setIsBidirectiona(true);
        originRoute = createRoute(rrep->getOriginatorAddr(), sourceAddr, newHopCount, seqNum, true, timeOut, metricType, metric, TOPSIS_UPDATE);
        orgRouteData = check_and_cast<LoadNgRouteData *>(originRoute->getProtocolData());
        actualizeCostRrep(rrep, orgRouteData);
     }
...
IRoute *LoadNgTopsis2::createRoute(const L3Address& destAddr, const L3Address& next,
    ...
    if (check == CHECK_UPDATE) {
        ...
        if (itTopsis != topsisResults.end()) {
            if (itTopsis->second.nextAddress != nextHop)
                throw cRuntimeError(""); // Change the value to the data in topsisData base, temporal solution
        }
    }
...
void LoadNgTopsis2::updateRoutingTable(IRoute *route, const L3Address& next, unsigned int hopCount, int64_t destSeqNum, bool isActive, simtime_t lifeTime, int metricType, const double & m, UpdateBehaviour check)
{
...
void LoadNgTopsis2::sendLoadNgPacket(const Ptr<LoadNgControlPacket>& packet, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
bool LoadNgTopsis2::actualizeMetric(const Ptr<LoadNgControlPacketWithTlv> &pkt, const double &val, double &newMetric) {
...
bool LoadNgTopsis2::setMetric(const Ptr<LoadNgControlPacketWithTlv> &pkt, const double & val) {
    auto metricPos = pkt->getTlvOptionsForUpdate().findByType(METRIC2);
    int metricType = -1;
    if (metricPos  != -1) {
...
bool LoadNgTopsis2::getMetric(const Ptr<LoadNgControlPacketWithTlv> &pkt, double &newMetric) {
    auto metricPos = pkt->getTlvOptionsForUpdate().findByType(METRIC2);
    int metricType = -1;
    if (metricPos != -1) {
        auto metricTlv = check_and_cast<LoadNgMetric2Option *>(pkt->getTlvOptionsForUpdate().getTlvOptionForUpdate(metricPos));
...
bool LoadNgTopsis2::checkCost(const Ptr<Rreq>& rreq, LoadNgRouteData * loadNgRouteData) {

    if (!loadNgRouteData->getStableRoute() && rreq->getStableRoute())
        return true;
...
void LoadNgTopsis2::actualizeCostUsingStatus(const NodeStatus &status, LoadNgRouteData * loadNgRouteData) {
    ...
    loadNgRouteData->setStableRoute(status.stationary && stationary);
    if (par("stationaryNetwork").boolValue())
        loadNgRouteData->setStableRoute(true);
}

void LoadNgTopsis2::actualizeCostUsingNeig(const NeigborElement &status, LoadNgRouteData * loadNgRouteData) {
    loadNgRouteData->setEnergy(status.energy);
...
void LoadNgTopsis2::actualizeCost(const Ptr<Rreq>& rreq, LoadNgRouteData * loadNgRouteData) {
    ...
    loadNgRouteData->setStableRoute(rreq->getStableRoute());
    if (par("stationaryNetwork").boolValue())
        loadNgRouteData->setStableRoute(true);
}

void LoadNgTopsis2::actualizeCostRrep(const Ptr<Rrep>& rrep, LoadNgRouteData * loadNgRouteData) {
    ...
    loadNgRouteData->setStableRoute(rrep->getStableRoute());
    if (par("stationaryNetwork").boolValue())
        loadNgRouteData->setStableRoute(true);
}


void LoadNgTopsis2::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, const L3Address& nextHopAddr, unsigned int timeToLive, const MacAddress &macSenderAddress)
{
    ...
    bool srcStationary = false;
    auto itStationary = std::find(stationaryNodes.begin(),stationaryNodes.end(),sourceAddr);
    if (itStationary != stationaryNodes.end())
        srcStationary = true;
    ...
    rreq->setStableRoute( rreq->getStableRoute() && stable);
    if (par("stationaryNetwork").boolValue()) {
        rreq->setStableRoute(true);
        stable = true;
    }
    ...
    if (itTopsis == topsisResults.end() || (itTopsis != topsisResults.end() && itTopsis->second.nextAddress == sourceAddr)) {
        ...
        if (itNeig != neighbors.end()) {
            ...
            if ((!previousHopRoute || previousHopRoute->getSource() != this) && !rreq->getAccumulate()) {
                ...
                if (previousHopRoute) {
                    ...
                    updateRoutingTable(previousHopRoute, sourceAddr, 1, loadNgRouteData->getDestSeqNum(), true, timeOut, metricType, metricNeg);
                }
            }
        }
        else {
            if ((!previousHopRoute || previousHopRoute->getSource() != this) && !rreq->getAccumulate()) {
                ...
                if (previousHopRoute) {
                    auto loadNgRouteData = check_and_cast<LoadNgRouteData *> (previousHopRoute->getProtocolData());
                    loadNgRouteData->setUsingTopsis(false);
                    loadNgRouteData->setStableRoute(stable);
                    updateRoutingTable(previousHopRoute, sourceAddr, 1, loadNgRouteData->getDestSeqNum(), true, timeOut, metricType, metricNeg);
                }
            }
        }
        ...
        simtime_t timeOut = simTime() + activeRouteTimeout;
        if (stable && checkStationary)
            timeOut = simTime() + par("stableRoutesPrimeTimeOut") * activeRouteTimeout;
        // TODO: check this, or should use the data of sourceAddr?
        if ((!previousHopRoute || previousHopRoute->getSource() != this) && !rreq->getAccumulate()) {
            // create without valid sequence number

            previousHopRoute = createRoute(sourceAddr, itTopsis->second.nextAddress, 1, -1, true, timeOut, metricType, metricNeg);
            auto loadNgRouteData = check_and_cast<LoadNgRouteData *> (previousHopRoute->getProtocolData());
            loadNgRouteData->setStableRoute(stable);
        }
        else {
            if (previousHopRoute) {
                auto loadNgRouteData = check_and_cast<LoadNgRouteData *> (previousHopRoute->getProtocolData());
                updateRoutingTable(previousHopRoute, itTopsis->second.nextAddress, 1, loadNgRouteData->getDestSeqNum(), true, timeOut, metricType, metricNeg);
                loadNgRouteData->setStableRoute(stable);
            }
        }
    }
    ...
    if (itTopsisOrg != topsisResults.end() && itTopsisOrg->second.nextAddress != sourceAddr) {
        ...
        rreq->setEnergy(energy);
        rreq->setSnir(snir);
        rreq->setStableRoute(itTopsisOrg->second.stable);
        if (par("stationaryNetwork").boolValue())
            rreq->setStableRoute(true);
    ...
    if (checkStationary && rreq->getStableRoute() && newLifeTime < simTime() + par("stableRoutesPrimeTimeOut") * activeRouteTimeout)
        newLifeTime = simTime() + par("stableRoutesPrimeTimeOut") * activeRouteTimeout;
    ...
    auto topsisCost = topsis.addCost(
            ...
            rreq->getHopCount(), rreq->getDelay(), rreq->getStableRoute());
    ...
    else if (oldSeqNum < rreq->getSeqNum() && rreq->getDestAddr().isUnspecified()  && rreq->getRrepRequest()) {
        auto rrep = createRREP(rreq);
        rrep->setHopLimit(par("maxHopLimit"));
        // send the rrep with a small jitter to avoid collisions.
        sendRREP(rrep, rreq->getOriginatorAddr(), 255, -1, *jitterPar);
...
#if 0
void LoadNgTopsis2::actualizeBreakRoutes() {
...
void LoadNgTopsis2::inectPacket(Packet *datagram)
{
...
void LoadNgTopsis2::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void LoadNgTopsis2::handleLinkBreakSendRERRSet(std::set<L3Address>& unreachableAddr, const L3Address& source)
{
...
void LoadNgTopsis2::handleLinkBreakSendRERR(const L3Address& unreachableAddr, const L3Address& source, const L3Address& destination)
{
...
const Ptr<Rerr> LoadNgTopsis2::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void LoadNgTopsis2::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void LoadNgTopsis2::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void LoadNgTopsis2::handleStopOperation(LifecycleOperation *operation)
{
    clearState();
}

void LoadNgTopsis2::handleCrashOperation(LifecycleOperation *operation)
{
    clearState();
}

void LoadNgTopsis2::clearState()
{
...
void LoadNgTopsis2::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void LoadNgTopsis2::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr)
{
...
void LoadNgTopsis2::forwardRREQ(const Ptr<Rreq>& rreq)
{
    ...
    if (!rreq->getTrigger() && !rreq->getBuild()) {
        ...
        if (checkStationary && !par("smartRREQ")) {
            next = routingTable->findBestMatchingRoute(rreq->getDestAddr());

            if (next) {
                auto destRouteData = check_and_cast<LoadNgRouteData *>(next->getProtocolData());
                if (!destRouteData->getStableRoute())
                    next = nullptr;
            }
        }
    ...
    double delay = 0;
    if (checkStationary && rreq->getStableRoute())
        delay = *jitterPar;
    else if (checkStationary && !rreq->getStableRoute() && jitterPar2)
        delay = *jitterPar2;
    else
        delay = *jitterPar;
    ...
    if (!next)
        sendLoadNgPacket(rreq, addressType->getBroadcastAddress(), rreq->getHopLimit(), delay);
    else
        sendLoadNgPacket(rreq, next->getNextHopAsGeneric(), rreq->getHopLimit(), delay);
}

void LoadNgTopsis2::completeRouteDiscoveryDelayed(const L3Address& target)
{
...
void LoadNgTopsis2::completeRouteDiscovery(const L3Address& target)
{
    EV_DETAIL << "Completing route discovery, originator " << getSelfIPAddress() << ", target " << target << endl;
    ASSERT(hasOngoingRouteDiscovery(target));
...
void LoadNgTopsis2::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void LoadNgTopsis2::regenerateLinkMap() {
   return;
}



void LoadNgTopsis2::checkNeigList(cMessage *timer)
{
    bool recompute = false;


    bool isStaticNetwork = par("stationaryNetwork").boolValue();
    for (auto it = neighbors.begin(); it != neighbors.end();) {
        int limit = par("allowedHelloLoss");
        if (isStaticNetwork || (checkStationary && stationary && it->second.stationary)) {
            limit += par("staticLinksAditionalLossHellos").intValue();
        }
        if (simTime() - it->second.lastNotification >= helloInterval * limit) {
...
const Ptr<Hello> LoadNgTopsis2::createHelloMessage()
{
...
void LoadNgTopsis2::sendHelloMessagesIfNeeded()
{
...
void LoadNgTopsis2::sendGratHelloMessages()
{
    if (!GRAT_HELLO)
        return;
...
bool LoadNgTopsis2::runTopsisNeigbors()
{
    double time = simTime().dbl();
...
bool LoadNgTopsis2::runTopsisNeigborsNow()
{
    TopsisLoadNgNeigbors2 topsis;
    topsis.setExperiment(par("matrixExperiment"));
   ...
   //     printf("");
    ...
    for (auto it = neighbors.begin(); it != neighbors.end();) {
        ...
        if (it->second.isBidirectional && !it->second.pendingConfirmation) {
            ...
            for (const auto &elem2: it->second.listNeigbours) {
                ...
                if (checkStationary) {
                    if (stationary && it->second.stationary && elem2.second.stationary)
                        stable = true;
                    else
                        stable = false;
                }
                if (elem2.second.isBidirectional && !elem2.second.pendingConfirmation) {
                    ...
                    if (itAlternate != neighbors.end()) {
                        ...
                        int limit = par("allowedHelloLoss");
                        if (isStaticNetwork || (checkStationary && stationary && itAlternate->second.stationary)) {
                            limit += par("staticLinksAditionalLossHellos").intValue();
                        }
                        if (simTime() - itAlternate->second.lastNotification >= helloInterval * limit) {
                            std::vector<IRoute *> routesToDelete;
                            for (int i = 0; i< routingTable->getNumRoutes(); i++) {
                                IRoute * route = routingTable->getRoute(i);
                            ...
                            for (const auto &elem3 : itAlternate->second.listNeigbours) {
                                if (checkStationary) {
                                    if (stationary &&it->second.stationary &&
                                            itAlternate->second.stationary
                                            && elem3.second.stationary)
                                        stable = true;
                                    else
                                        stable = false;
                                }
    ...
    for (const auto &elem : topsisResults) {
        ...
        simtime_t newLifeTime = it->second.lastNotification + limit * minHelloInterval;
        ...
        if (!route || route->getSource() != this) {
            ...
            routeData->setStableRoute(elem.second.stable);
            if (par("stationaryNetwork").boolValue())
                routeData->setStableRoute(true);
...
void LoadNgTopsis2::runTopsisNeigborsNowCheckTopsis()
{
    TopsisLoadNgNeigbors2 topsis;
    topsis.setExperiment(par("matrixExperiment"));
    lastTopsisCompute = simTime();
...
bool LoadNgTopsis2::runTopsisNeigborsNowTwoHops()
{
    TopsisLoadNgNeigbors2 topsis;
    topsis.setExperiment(par("matrixExperiment"));
    lastTopsisCompute = simTime();
...
void LoadNgTopsis2::handleHelloMessage(const Ptr<const Hello>& helloMessage, const Ptr<const SignalPowerInd> & powerInd, const Ptr<const SnirInd>& snirInd, const MacAddress &macSenderAddress)
{
...
//        forceRecompute = true;
   ...
   // int numRoutes = routingTable->getNumRoutes();
    ...
    bool stable = true;
    if (checkStationary) {
        stable = stationary && it->second.stationary;
    }
    ...
    if (it->second.pendingConfirmation == false && it->second.isBidirectional) {
        ...
        if (!routeHelloOriginator || routeHelloOriginator->getSource() != this) {
            ...
            routeData->setIsBidirectiona(true);
            routeData->setStableRoute(stable);
        }
        else {
            LoadNgRouteData *routeData = check_and_cast<LoadNgRouteData *>(routeHelloOriginator->getProtocolData());
            simtime_t lifeTime = routeData->getLifeTime();
...
void LoadNgTopsis2::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        IRoute *route = routingTable->getRoute(i);
        if (route->getSource() == this) {
...
void LoadNgTopsis2::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result LoadNgTopsis2::datagramForwardHook(Packet *datagram)
{
...
void LoadNgTopsis2::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr, const L3Address &destAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void LoadNgTopsis2::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool LoadNgTopsis2::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        LoadNgRouteData *routeData = check_and_cast<LoadNgRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> LoadNgTopsis2::createRREPACK()
{
...
void LoadNgTopsis2::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendLoadNgPacket(rrepACK, destAddr, 100, 0);
}

void LoadNgTopsis2::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void LoadNgTopsis2::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void LoadNgTopsis2::handleBlackListTimer()
{
...
void LoadNgTopsis2::finish(){
    if (StableSourceDest) {
        recordScalar("StablesRutes", StableSourceDest);
        StableSourceDest = 0;
    }
    if (NoStableSourceDest) {
        recordScalar("StablesNoRoutes", NoStableSourceDest);
        NoStableSourceDest = 0;
    }
}
