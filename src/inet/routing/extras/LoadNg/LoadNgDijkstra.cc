...
#include "inet/routing/extras/LoadNg/LoadNgDijkstra.h"
...
// TODO: Estudiar que porras pasa con el tema de lastNotification, por que funciona mejor con el error de actualiar este campo cuando no toca.

namespace inet {
namespace inetmanet {

simsignal_t LoadNgDijkstra::recomputeSignal = registerSignal("LoadNgRecomputeSignal");


Define_Module(LoadNgDijkstra);
...
void LoadNgDijkstra::initialize(int stage)
{
...
void LoadNgDijkstra::actualizeDelayed(Packet *pkt) {
...
void LoadNgDijkstra::handleMessageWhenUp(cMessage *msg)
{
    checkNeigList(msg);
...
INetfilter::IHook::Result LoadNgDijkstra::ensureRouteForDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
LoadNgDijkstra::LoadNgDijkstra()
{
}

bool LoadNgDijkstra::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void LoadNgDijkstra::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address LoadNgDijkstra::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void LoadNgDijkstra::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void LoadNgDijkstra::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void LoadNgDijkstra::sendRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive, const double &cost)
{
...
const Ptr<Rreq> LoadNgDijkstra::createRREQ(const L3Address& destAddr)
{
...
const Ptr<Rrep> LoadNgDijkstra::createRREPSp(const L3Address &dest, const SpecialPath &path)
{
...
const Ptr<Rrep> LoadNgDijkstra::createRREP(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
...
void LoadNgDijkstra::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr, const MacAddress &macSenderAddress)
{
...
void LoadNgDijkstra::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, int64_t destSeqNum, bool isActive, simtime_t lifeTime, int metricType, const double & metric)
{
...
void LoadNgDijkstra::sendLoadNgPacket(const Ptr<LoadNgControlPacket>& packet, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
bool LoadNgDijkstra::actualizeMetric(const Ptr<LoadNgControlPacketWithTlv> &pkt, const double &val, double &newMetric) {
...
bool LoadNgDijkstra::setMetric(const Ptr<LoadNgControlPacketWithTlv> &pkt, const double & val) {
    auto metricPos = pkt->getTlvOptionsForUpdate().findByType(METRIC2);
    int metricType = -1;
    if (metricPos  != -1) {
...
bool LoadNgDijkstra::getMetric(const Ptr<LoadNgControlPacketWithTlv> &pkt, double &newMetric) {
    auto metricPos = pkt->getTlvOptionsForUpdate().findByType(METRIC2);
    int metricType = -1;
    if (metricPos != -1) {
        auto metricTlv = check_and_cast<LoadNgMetric2Option *>(pkt->getTlvOptionsForUpdate().getTlvOptionForUpdate(metricPos));
...
bool LoadNgDijkstra::checkCost(const Ptr<Rreq>& rreq, LoadNgRouteData * loadNgRouteData) {
...
void LoadNgDijkstra::actualizeCost(const Ptr<Rreq>& rreq, LoadNgRouteData * loadNgRouteData) {
...
void LoadNgDijkstra::actualizeCostUsingStatus(const NodeStatus *status, LoadNgRouteData * loadNgRouteData) {
...
void LoadNgDijkstra::actualizeCostUsingNeig(const NeigborElement *status, LoadNgRouteData * loadNgRouteData) {
    loadNgRouteData->setEnergy(status->energy);
...
void LoadNgDijkstra::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive, const MacAddress &macSenderAddress)
{
...
IRoute *LoadNgDijkstra::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
#if 0
void LoadNgDijkstra::actualizeBreakRoutes() {
...
void LoadNgDijkstra::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void LoadNgDijkstra::handleLinkBreakSendRERR(const L3Address& unreachableAddr, const L3Address& source, const L3Address& destination)
{
...
const Ptr<Rerr> LoadNgDijkstra::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void LoadNgDijkstra::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void LoadNgDijkstra::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void LoadNgDijkstra::handleStopOperation(LifecycleOperation *operation)
{
    clearState();
}

void LoadNgDijkstra::handleCrashOperation(LifecycleOperation *operation)
{
    clearState();
}

void LoadNgDijkstra::clearState()
{
...
void LoadNgDijkstra::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void LoadNgDijkstra::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void LoadNgDijkstra::forwardRREQ(const Ptr<Rreq>& rreq, unsigned int timeToLive)
{
...
void LoadNgDijkstra::completeRouteDiscoveryDelayed(const L3Address& target)
{
...
void LoadNgDijkstra::completeRouteDiscovery(const L3Address& target)
{
    EV_DETAIL << "Completing route discovery, originator " << getSelfIPAddress() << ", target " << target << endl;
    ASSERT(hasOngoingRouteDiscovery(target));
...
void LoadNgDijkstra::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void LoadNgDijkstra::regenerateLinkMap() {
    if (!useCommonData)
        return;
...
void LoadNgDijkstra::rebuildDijkstraWihtHellos()
{
    if (dijkstra == nullptr)
        return;
    dijkstra->clearAll();
...
void LoadNgDijkstra::runDijkstra()
{
...
void LoadNgDijkstra::runDijkstraKs()
{
...
void LoadNgDijkstra::checkNeigList(cMessage *timer)
{
...
const Ptr<Hello> LoadNgDijkstra::createHelloMessage()
{
...
void LoadNgDijkstra::sendHelloMessagesIfNeeded()
{
...
#if 0
void LoadNgDijkstra::handleHelloMessage(const Ptr<const Hello>& helloMessage, SignalPowerInd * powerInd, SnirInd * snirInd)
{
    double power = NaN;
    double snir = NaN;
    if (powerInd)
...
void LoadNgDijkstra::handleHelloMessage(const Ptr<const Hello>& helloMessage, const Ptr<const SignalPowerInd> &powerInd,const Ptr<const SnirInd>  &snirInd, const MacAddress &macSenderAddress)
{
    double power = NaN;
    double snir = NaN;
    if (powerInd)
    ...
    */
    ...
    int numRoutes = routingTable->getNumRoutes();
    if (dijkstraTotal && ((int)dijkstraTotal->getNumRoutes() < (int)nodesData.size() - 1 || (int)dijkstraTotal->getNumRoutes() != numRoutes)) {
        emit(recomputeSignal, this);
    }
...
void LoadNgDijkstra::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        IRoute *route = routingTable->getRoute(i);
        if (route->getSource() == this) {
...
void LoadNgDijkstra::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result LoadNgDijkstra::datagramForwardHook(Packet *datagram)
{
...
void LoadNgDijkstra::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr, const L3Address &destAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void LoadNgDijkstra::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool LoadNgDijkstra::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        LoadNgRouteData *routeData = check_and_cast<LoadNgRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> LoadNgDijkstra::createRREPACK()
{
...
void LoadNgDijkstra::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendLoadNgPacket(rrepACK, destAddr, 100, 0);
}

void LoadNgDijkstra::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void LoadNgDijkstra::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void LoadNgDijkstra::handleBlackListTimer()
{
