...
class INET_API LoadNgTopsis2 : public RoutingProtocolBase, public NetfilterBase::HookBase, public cListener
{
  ...
  protected:
    ...
    std::map<L3Address, TopsisLoadNgNeigbors2::Cost> topsisResults; // the information obtained after use topsis
    std::map<L3Address, TopsisLoadNgNeigbors2::Cost> topsisResultsTwoHops; // the information obtained after use topsis
    std::map<L3Address, TopsisLoadNgNeigbors2::Cost> topsisResultsCheck; // the information obtained after use topsis
    std::map<MacAddress, L3Address > macToIpAddress;

    struct CostInfo {
        int index = -1;
        std::deque<TopsisLoadNgNeigbors2::Cost> list;
        int64_t seqNum;
...
//end unidirectional structs
    ...
    uint64_t &StableSourceDest = SIMULATION_SHARED_COUNTER(StableSourceDest);
    uint64_t &NoStableSourceDest = SIMULATION_SHARED_COUNTER(StableSourceDest);
    ...
    double etxPenalty = 1;
    bool checkStationary = false;
    std::vector<L3Address> &stationaryNodes = SIMULATION_SHARED_VARIABLE(stationaryNodes);
    std::vector<L3Address> &mobileNodes = SIMULATION_SHARED_VARIABLE(mobileNodes);
