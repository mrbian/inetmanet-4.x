...
#include "inet/routing/extras/LoadNg/LoadNgCt.h"
...
void LoadNgCt::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        minHelloJitter = par("helloMinJitter").doubleValue();
        maxHelloJitter = par("helloMaxJitter").doubleValue();
...
void LoadNgCt::actualizeDelayed(Packet *pkt) {
...
void LoadNgCt::handleMessageWhenUp(cMessage *msg)
{
...
INetfilter::IHook::Result LoadNgCt::ensureRouteForDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
bool LoadNgCt::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}


void LoadNgCt::periodicRreqTigger()
{
    EV_INFO << "Sending periodic RREQ  "<< endl;
    auto rreqTr = createRREQ(this->getSelfIPAddress());
    rreqTr->setHopLimit(periodicRreqPropagation);
    rreqTr->setTrigger(true);
    // Don't program rrep wait timers. Send immediately
    sendLoadNgPacket(rreqTr, addressType->getBroadcastAddress(), par("periodicRreqPropagation").intValue(), 0);
    rreqCount++;
    scheduleAt(simTime() + par("PeriodicRreq"), periodicRreqTimer);
    EV_INFO << "Sending periodic RREQ  "<< endl;

    // Program to sent the RREQ build in 2xnet transversal time
    double delay = 2*par("netTransversalTime").doubleValue();
    auto rreqBl = createRREQ(this->getSelfIPAddress());
    rreqBl->setHopLimit(periodicRreqPropagation);
    rreqBl->setBuild(true);
    rreqCount++;
    sendLoadNgPacket(rreqBl, addressType->getBroadcastAddress(), par("periodicRreqPropagation").intValue(), delay);
}

void LoadNgCt::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address LoadNgCt::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void LoadNgCt::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void LoadNgCt::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void LoadNgCt::sendRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive, const double &cost, const double &delay)
{
...
const Ptr<Rreq> LoadNgCt::createRREQ(const L3Address& destAddr)
{
...
const Ptr<Rrep> LoadNgCt::createRREPSp(const L3Address &dest, const SpecialPath &path)
{
...
const Ptr<Rrep> LoadNgCt::createRREP(const Ptr<Rreq>& rreq)
{
...
const Ptr<Hello> LoadNgCt::createHelloMessage()
{
    ...
    std::vector<NeigborData> data;
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        auto entry = routingTable->getRoute(i);
        if (entry->getSource() != this)
            continue;
        if (entry->getNextHopAsGeneric() != entry->getDestinationAsGeneric())
            continue;
        auto loadNgRouteData = check_and_cast<LoadNgRouteData *> (entry->getProtocolData());
        if (simTime() - loadNgRouteData->getLastUpdateTriger() > 2*maxHelloJitter)
            continue;
        NeigborData nData;
        nData.setAddr(entry->getDestinationAsGeneric());
    ...
    if (!(data.empty())) {
        ...
        helloMessage->setNeighAddrsArraySize(data.size());
        for (unsigned int i = 0; i < data.size(); i++){
            s+=1;
            helloMessage->setNeighAddrs(i, data[i]);
        }
        // size
...
void LoadNgCt::sendHelloMessagesIfNeeded()
{
    simtime_t nextHello = simTime() + 2;
...
void LoadNgCt::handleHelloMessage(const Ptr<const Hello>& helloMessage, const Ptr<const SignalPowerInd> & powerInd, const Ptr<const SnirInd> & snirInd, const MacAddress &macSenderAddress)
{

    const L3Address& helloOriginatorAddr = helloMessage->getOriginatorAddr();
    auto seqNum = helloMessage->getSeqNum();
    ...
    IRoute *neighRoute = routingTable->findBestMatchingRoute(helloOriginatorAddr);
    if (!neighRoute || neighRoute->getSource() != this) {
        // create without valid sequence number
        neighRoute = createRoute(helloOriginatorAddr, helloOriginatorAddr, 1, seqNum, true, simTime() + activeRouteTimeout, HOPCOUNT, 1, weak);
    }
    else {
        updateRoutingTable(neighRoute, helloOriginatorAddr, 1, seqNum, true, simTime() + activeRouteTimeout, HOPCOUNT, 1, weak);
    }

    auto loadNgRouteData = check_and_cast<LoadNgRouteData *> (neighRoute->getProtocolData());
    auto itAux = nodeList.find(this->getSelfIPAddress());
    if (itAux == nodeList.end()) {
        loadNgRouteData->setIsBidirectiona(false);
    }
    else {
        loadNgRouteData->setIsBidirectiona(true);
    }

    if (loadNgRouteData->getIsBidirectiona()) {
        // if the link is biDirectional, delete from blacklist
        auto blackListIt = blacklist.find(helloOriginatorAddr);
        if (blackListIt != blacklist.end())
            blacklist.erase(blackListIt);
    }
    else {
        blacklist[helloOriginatorAddr] = simTime() + blacklistTimeout;
    }
}


void LoadNgCt::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr, const MacAddress &macSenderAddress, SignalPowerInd * powerInd, SnirInd * snirInd)
{
...
void LoadNgCt::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, int64_t destSeqNum, bool isActive, simtime_t lifeTime, int metricType, const double & metric, const int &weak)
{
...
void LoadNgCt::sendLoadNgPacket(const Ptr<LoadNgControlPacket>& packet, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
bool LoadNgCt::actualizeMetric(const Ptr<LoadNgControlPacketWithTlv> &pkt, const double &val, double &newMetric) {
...
bool LoadNgCt::setMetric(const Ptr<LoadNgControlPacketWithTlv> &pkt, const double & val) {
    auto metricPos = pkt->getTlvOptionsForUpdate().findByType(METRIC2);
    int metricType = -1;
    if (metricPos  != -1) {
...
bool LoadNgCt::getMetric(const Ptr<LoadNgControlPacketWithTlv> &pkt, double &newMetric) {
    auto metricPos = pkt->getTlvOptionsForUpdate().findByType(METRIC2);
    int metricType = -1;
    if (metricPos != -1) {
        auto metricTlv = check_and_cast<LoadNgMetric2Option *>(pkt->getTlvOptionsForUpdate().getTlvOptionForUpdate(metricPos));
...
void LoadNgCt::actualizeCost(const Ptr<Rreq>& rreq, LoadNgRouteData * loadNgRouteData) {
...
void LoadNgCt::actualizeCostUsingStatus(const NodeStatus *status, LoadNgRouteData * loadNgRouteData) {
...
void LoadNgCt::actualizeCostUsingNeig(const NeigborElement *status, LoadNgRouteData * loadNgRouteData) {
    loadNgRouteData->setEnergy(status->energy);
...
void LoadNgCt::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive, const MacAddress &macSenderAddress, SignalPowerInd * powerInd, SnirInd * snirInd)
{
    ...
    if (rreq->getBuild()) {
        IRoute *previousHopRoute = routingTable->findBestMatchingRoute(sourceAddr);
        if (previousHopRoute == nullptr)
            return;
        // set bidirectional to false
        auto routeData = check_and_cast<LoadNgRouteData *>(previousHopRoute->getProtocolData());
        if (simTime() - routeData->getLastUpdateTriger() > maxHelloJitter)
            return;
        if (!routeData->getIsBidirectiona())
            return;
    }
    ...
    if (rreq->getTrigger()) {
        // set bidirectional to false
        auto routeData = check_and_cast<LoadNgRouteData *>(previousHopRoute->getProtocolData());
        routeData->setIsBidirectiona(false);
        routeData->setLastUpdateTriger(simTime());
        if (!helloMsgTimer->isScheduled()) {
            scheduleAt(simTime()+par("helloJitter"), helloMsgTimer);
        }
    }
    ...
    else if (oldSeqNum < rreq->getSeqNum() && rreq->getBuild() && rreq->getRrepRequest()) {
        auto rrep = createRREP(rreq);
        rrep->setHopLimit(par("maxHopLimit"));
...
IRoute *LoadNgCt::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
#if 0
void LoadNgCt::actualizeBreakRoutes() {
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
...
void LoadNgCt::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void LoadNgCt::handleLinkBreakSendRERR(const L3Address& unreachableAddr, const L3Address& source, const L3Address& destination)
{
...
const Ptr<Rerr> LoadNgCt::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void LoadNgCt::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void LoadNgCt::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
    // RFC 5148:
...
void LoadNgCt::handleStopOperation(LifecycleOperation *operation)
{
    clearState();
}

void LoadNgCt::handleCrashOperation(LifecycleOperation *operation)
{
    clearState();
}

void LoadNgCt::clearState()
{
...
void LoadNgCt::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void LoadNgCt::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr)
{
...
void LoadNgCt::forwardRREQ(const Ptr<Rreq>& rreq)
{
...
void LoadNgCt::completeRouteDiscoveryDelayed(const L3Address& target)
{
...
void LoadNgCt::completeRouteDiscovery(const L3Address& target)
{
    EV_DETAIL << "Completing route discovery, originator " << getSelfIPAddress() << ", target " << target << endl;
    ASSERT(hasOngoingRouteDiscovery(target));
...
void LoadNgCt::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void LoadNgCt::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result LoadNgCt::datagramForwardHook(Packet *datagram)
{
...
void LoadNgCt::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr, const L3Address &destAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void LoadNgCt::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool LoadNgCt::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        LoadNgRouteData *routeData = check_and_cast<LoadNgRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> LoadNgCt::createRREPACK()
{
...
void LoadNgCt::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendLoadNgPacket(rrepACK, destAddr, 100, 0);
}

void LoadNgCt::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void LoadNgCt::handleRREPACKTimer(const L3Address & addr)
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void LoadNgCt::handleBlackListTimer()
{
...
LoadNgCt::~LoadNgCt()
{
    ...
    cancelAndDelete(periodicRreqTimer);
