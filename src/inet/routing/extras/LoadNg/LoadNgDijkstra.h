...
#ifndef __INET_LOADNGDIJKSTRA_H
#define __INET_LOADNGDIJKSTRA_H
...
class INET_API LoadNgDijkstra : public RoutingProtocolBase, public NetfilterBase::HookBase, public cListener
{
protected:
...
//end unidirectional structs
    ...
    Dijkstra *dijkstraTotal = nullptr;
  ...
  protected:
    ...
    void updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, int64_t destSeqNum, bool isActive, simtime_t lifeTime, int metricType, const double & metric);
    IRoute *createRoute(const L3Address& destAddr, const L3Address& nextHop, unsigned int hopCount, int64_t destSeqNum, bool isActive, simtime_t lifeTime, int metricType, const double &metric);
    ...
    void handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive, const MacAddress &);
    void handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr);
    void handleHelloMessage(const Ptr<const Hello>& helloMessage, const Ptr<const SignalPowerInd> & ,const Ptr<const SnirInd>  &, const MacAddress &macSenderAddress);
    ...
    virtual void rebuildDijkstraWihtHellos();
